    ...
    <div class="max-w-md w-full space-y-8 bg-white p-10 rounded-2xl shadow-sm border border-gray-100">
        ...
        <?php if (\App\Core\Session::hasFlash('error')): ?>
            <div class="bg-red-50 text-red-600 p-4 rounded-xl text-sm border border-red-100 flex items-center">
                <i class="fa-solid fa-circle-exclamation mr-2"></i><?= htmlspecialchars(\App\Core\Session::getFlash()) ?>
            </div>
        <?php endif; ?>

        <?php if (\App\Core\Session::hasFlash('success')): ?>
            <div class="bg-green-50 text-green-600 p-4 rounded-xl text-sm border border-green-100 flex items-center">
                <i class="fa-solid fa-circle-check mr-2"></i><?= htmlspecialchars(\App\Core\Session::getFlash()) ?>
            </div>
        <?php endif; ?>
        ...
        <div class="flex items-center justify-between mt-4">
            <span class="text-sm text-gray-500">¿No tienes cuenta? <a href="<?= BASE_URL ?>/register" class="font-bold text-primary hover:text-orange-600">Regístrate</a></span>
