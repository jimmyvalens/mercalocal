...
// Inicializamos los totales globales para evitar el error "Undefined variable"
$totalComercios = $totalComercios ?? 0;
$totalActivos = $totalActivos ?? 0;
$totalInactivos = $totalInactivos ?? 0;
...
<div class="max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-12">
    <div class="flex flex-col md:flex-row gap-8 w-full min-w-0">
        ...
        <div class="flex-1 min-w-0">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 text-center">
                    <p class="text-sm font-medium text-gray-500 mb-2">Todos los Comercios</p>
                    <p class="text-3xl font-bold text-gray-900"><?= $totalComercios ?></p>
                </div>
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 text-center">
                    <p class="text-sm font-medium text-gray-500 mb-2">Comercios Inactivos</p>
                    <p class="text-3xl font-bold text-red-600"><?= $totalInactivos ?></p>
                </div>

                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 text-center">
                    <p class="text-sm font-medium text-gray-500 mb-2">Todos los Productos</p>
            ...
            <form method="GET" action="<?= BASE_URL ?>/admin/businesses" class="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 mb-8 flex flex-col lg:flex-row gap-4 items-end">
                ...
                <div class="w-full lg:w-48">
                    ...
                    <select name="status" id="status" class="block w-full py-2 pl-3 pr-10 text-base border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-xl transition-colors cursor-pointer">
                ...
                <div class="w-full lg:w-48">
                    ...
                    <select name="category" id="category" class="block w-full py-2 pl-3 pr-10 text-base border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-xl transition-colors cursor-pointer">
                ...
                <div class="flex gap-2 w-full lg:w-auto">
                    <button type="submit" class="w-full lg:w-auto bg-gray-800 text-white px-5 py-2 rounded-xl hover:bg-gray-900 transition-colors font-medium cursor-pointer">
                        Filtrar
                    </button>
                    <?php if (!empty($_GET['search']) || !empty($_GET['status']) || !empty($_GET['category'])): ?>
                        <a href="<?= BASE_URL ?>/admin/businesses" class="w-full lg:w-auto text-center bg-gray-100 text-gray-600 px-5 py-2 rounded-xl hover:bg-gray-200 transition-colors font-medium cursor-pointer">
                            Limpiar
                        </a>
                    <?php endif; ?>
                </div>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden w-full max-w-full">
                <div class="overflow-x-auto">
                    <table class="min-w-225 divide-y divide-gray-100">
                                        ...
                                        <td class="px-6 py-4 text-center">
                                            <div class="flex items-center justify-center gap-4">
                                                ...
                                                <form action="<?= BASE_URL ?>/admin/business/<?= $business['id'] ?>/delete" method="POST" class="inline-block form-eliminar">
                                                    ...
                                                    <button type="submit" class="inline-flex items-center justify-center w-8 h-8 bg-red-50 text-red-600 hover:bg-red-600 hover:text-white rounded-lg transition-colors shadow-sm cursor-pointer" title="Eliminar">
            ...
            <?php if ($totalPages > 1): ?>
                <div class="flex items-center justify-between border-t border-gray-200 bg-white px-4 py-3 sm:px-6 mt-4 rounded-2xl shadow-sm border">
