...
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    <div class="mb-8 flex items-center gap-4">
        <a href="<?= BASE_URL ?>/business/dashboard/products" class="text-gray-500 hover:text-primary transition-colors flex items-center gap-2">
        ...
        <form action="<?= isset($product) ? BASE_URL . '/business/dashboard/products/' . $product->id . '/update' : BASE_URL . '/business/dashboard/products/store' ?>" method="POST" enctype="multipart/form-data" class="p-8 space-y-6">
            ...
            <div class="grid grid-cols-1 gap-6">
                ...
                <div>
                    ...
                    <input name="nombre" value="<?= htmlspecialchars($product->nombre ?? '') ?>" required
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div class="flex items-end pb-3">
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" name="activo" value="1" class="sr-only peer" <?= (isset($product) && !$product->activo) ? '' : 'checked' ?>>
                            <div class="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-orange-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:width-5 after:transition-all peer-checked:bg-primary"></div>
                            <span class="ml-3 text-sm font-bold text-gray-700">Producto visible</span>
                        </label>
                    </div>
                </div>
                <div class="mb-4">
                    <label class="block text-sm font-bold text-gray-700 mb-2">Imagen del Producto <span class="text-red-500">*</span></label>
                    <input type="file" name="imagen" accept="image/*" class="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 focus:outline-none focus:border-primary">
                </div>
